...
#
# Ported from voicebox `backend/services/refinement.py` at the commit
# pinned in /voicebox-pin.txt. JustVoice adaptations: the LLM call routes
# through the provider registry's feature-pin dispatch (feature
# "dictation_refine") instead of voicebox's local-Qwen backend; the
# few-shot examples ride along as chat turns the same way.
"""Transcript refinement — turns raw STT output into a cleaner version
by running it through the pinned LLM with a toggle-driven system prompt.

The prompt is assembled server-side from boolean flags so the UI exposes
friendly toggles ("Smart cleanup", "Remove self-corrections") rather
than a raw prompt editor.
"""
...
# the transcript. Whisper occasionally loops content when audio trails
# off — "URL URL URL…", "thanks for watching" × N, or CJK loops with
# no spaces. Small refine models truncate legitimate output to "make
# room" for the loop; big ones echo it verbatim. Strip deterministically.
_REPETITION_RUN_THRESHOLD = 6
# Upper bound on the repeating-unit length the character-level pass
# detects — covers observed Whisper hallucination phrases (~41 chars max)
# while keeping coincidental long-phrase repetition under the threshold.
...
def collapse_repetitive_artifacts(text: str, min_run: int = _REPETITION_RUN_THRESHOLD) -> str:
    """Strip STT-artifact loops. Word-level pass catches single-token
    runs; character-level pass catches multi-word and CJK loops. Both
    preserve rhetorical repetition under the threshold ("no, no, no")."""
    collapsed = _collapse_word_runs(text, min_run)
    return _collapse_character_runs(collapsed, min_run)
...
For example, "it has three hundred k no no no actually four hundred k stars" yields "It has 400k stars.\""""
...
# inline examples inside the system prompt make small models echo the
# example output for unrelated inputs.
REFINEMENT_EXAMPLES: list[tuple[str, str]] = [
    (
        "so um i was thinking like maybe we could you know try that new place tonight if you're free",
...
async def refine_transcript(transcript: str, flags: RefinementFlags) -> tuple[str, str]:
    """Run the transcript through the pinned LLM.

    Returns (refined_text, model_label) so callers can persist which
    model produced the refinement. Raises LLMNotConfiguredError when no
    provider is registered (caller maps to 501).
    """
    from .engines.llm.base import LLMMessage
    ...
    adapter, model, _tier = resolve_pin(get_state().settings.get(), "dictation_refine")
    ...
    return resp.text.strip(), f"{adapter.id}:{model}"
