...
Per RULE #3 (upstream parity is file-by-file): this is a clean
re-implementation of JustWrite's services/openai-compat.js focused on
the chat-completions surface only. Provider-specific quirks (Ollama's
/api/chat, Anthropic's Messages API, Gemini's generativelanguage shape)
live in their own adapter modules.
"""
...
# Per-provider default base URLs. Used when settings.engines.llm[].base_url
        ...
        *,
        ...
        if not self._base_url:
            raise ValueError(
                ...
                f"and no default available — set base_url in settings.engines.llm"
            )
