...
Per JustWrite's services/openai-compat.js Ollama handling: think mode
only fires on /api/chat, not the OpenAI-compat /v1 endpoint Ollama also
exposes. We use the native endpoint exclusively so tier=reasoned works.
"""
