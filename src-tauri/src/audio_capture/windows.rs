...
//
// Originally from https://github.com/jamiepine/voicebox/blob/b35b90961d5bc83a8b4e96e8b6ccde2a03152ff9/tauri/src-tauri/src/audio_capture/windows.rs
    ...
    *stop_tx.lock().unwrap() = Some(tx);
    ...
    thread::spawn(move || {
        ...
        *channels_arc.lock().unwrap() = mix_format.get_nchannels();
        ...
        // For loopback: get Render device, initialize with Capture direction —
        // wasapi crate sets AUDCLNT_STREAMFLAGS_LOOPBACK internally.
...
pub async fn stop_capture(state: &AudioCaptureState) -> Result<String, String> {
    ...
    // Convert to WAV and base64-encode for the caller
...
fn base64_encode(data: &[u8]) -> String {
    // Simple base64 without pulling in another dep since hound already pulled
    // in the stdlib. Use the standard alphabet.
