...
Phase 2 / Slice 6 of the Profile-kill plan. Ported from JustWrite's
modelMeta.js with the same heuristic-match-on-model-id approach.

Each tier carries:
  - system_key: which prompt body the extraction backend should send
    (Slice 7 hooks this up; for now it surfaces through resolve_pin
    so the dispatch layer knows which tier to use).
